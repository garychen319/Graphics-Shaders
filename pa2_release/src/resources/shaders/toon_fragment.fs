...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal)
{
    ...
    // Diffuse Light
	//Round to 4 different values depending on interval
    float factor = max(dot(normal, to_light_dir), 0.0);
    if (factor < 0.1) factor = 0.0;
    else if (factor < 0.3) factor = 0.3;
    else if (factor < 0.6) factor = 0.6;
    else factor = 1.0;
    
    diffuseColour = vec4(light_colour, 1.0) * light_intensity * factor;
